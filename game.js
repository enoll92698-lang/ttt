...
function drawFood() {
  ...
  ctx.fillStyle = ft.glow; ctx.fill();
  ...
  if (isGold) {
    ctx.shadowBlur = 0;
    ctx.fillStyle = '#333';
    ctx.fillRect(food.x * CELL + 2, food.y * CELL - 6, CELL - 4, 3);
    ctx.fillStyle = '#ffd700';
    ctx.fillRect(food.x * CELL + 2, food.y * CELL - 6, (CELL - 4) * (remaining / GOLD_TTL), 3);
  }
  ctx.restore();
}
...
function spawnFood() {
  ...
  food = { ...pos, type: pickFoodType(), spawnedAt: performance.now() };
}

function addObstacles(count) {
  for (let i = 0; i < count; i++) obstacles.push(randomEmpty());
}

function getHSKey() { return `snakeHS_${gameMode}`; }
...
function tick() {
  ...
  if (gameMode === 'nowalls') {
    ...
    if (nx < 0 || nx >= GRID || ny < 0 || ny >= GRID) { endGame('collision'); return; }
  }

  if (snake.some(s => s.x === nx && s.y === ny))     { endGame('collision'); return; }
  if (obstacles.some(o => o.x === nx && o.y === ny)) { endGame('collision'); return; }
...
function startTickLoop() {
  function loop(ts) {
    ...
    if (ts - lastTick >= getSpeed()) { tick(); lastTick = ts; }
    updateEffectBar();
    render();
  }
...
function pvStep() {
  ...
  const options = shuffle([pvDir, turnLeft(pvDir), turnRight(pvDir)]);
...
function pvDraw(pc) {
  pc.fillStyle = C.bg;
  pc.fillRect(0, 0, PV_GRID * PV_CELL, PV_GRID * PV_CELL);
...
function initPreview() {
  ...
  pvc.width  = PV_GRID * PV_CELL;
  pvc.height = PV_GRID * PV_CELL;
